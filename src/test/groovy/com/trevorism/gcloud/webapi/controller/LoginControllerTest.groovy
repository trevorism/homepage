...
class LoginControllerTest {
    ...
    void setup() {
        ...
        loginController.userSessionService = [getToken: { lr -> lr.username }, getUserFromToken: { token -> new User(username: token) }, generateForgotPasswordLink: { val -> if (!val) throw new RuntimeException() }, resetPassword: { r -> if (!r) throw new RuntimeException() }] as UserSessionService
    }
